...
Bối cảnh: Phía Frontend (ReactJS) đã tích hợp thư viện payos-checkout. Quy trình hoạt động như sau:

Người dùng bấm "Thanh toán".
Frontend gọi API tạo đơn hàng (
createOrder
) -> có orderId.
Frontend gọi tiếp API lấy link thanh toán PayOS (
createPayOSLink
).
Backend cần xử lý tạo link thanh toán và trả về checkoutUrl.
Frontend dùng checkoutUrl để hiển thị popup thanh toán của PayOS.
...
**Endpoint:** `POST /api/payment/payos/create-payment-link/:maKH`

**Logic xử lý:**
- Lấy `maKH` (mã khách hàng) từ params.
- Query Database để tìm đơn hàng của khách hàng với điều kiện `MaTTDH = 6` (trạng thái chờ xử lý/giỏ hàng).
- Lấy thông tin đơn hàng (Tổng tiền, Chi tiết sản phẩm...).
- Tạo payload gửi sang PayOS. Lưu ý: orderCode của PayOS yêu cầu là số nguyên (Number) và không quá lớn (Safe Integer). Generate một mã số duy nhất dựa trên timestamp.
- Gọi hàm `payOS.paymentRequests.create(body)`.
...
Cấu trúc Order: Cần lưu lại orderCode (mã sinh ra cho PayOS) vào Database tương ứng với đơn hàng (orderId) để khi nhận Webhook có thể đối soát cập nhật trạng thái đúng đơn hàng.
Cấu trúc Response: API tạo link thanh toán bắt buộc phải trả về object có dạng:
json
{
   "success": true,
   "data": {
...
CORS: Đảm bảo cấu hình CORS để Frontend (localhost hoặc domain prod) có thể gọi API này.