...
const DonDatHangController = {
  ...
  getByStatus: async (req, res) => {
    try {
      ...
      if (!status) {
        return response.error(res, null, 'Vui lòng cung cấp trạng thái đơn hàng', 400);
      }
      ...
      if (parsedStatus !== 'all' && (isNaN(parsedStatus) || parsedStatus < 0)) {
        return response.error(res, null, 'Trạng thái đơn hàng không hợp lệ', 400);
      }

      const data = await DonDatHangService.getByStatus(parsedStatus, parsedPage, parsedLimit);
      return response.success(res, data, 'Lấy danh sách đơn hàng theo trạng thái thành công');
    } catch (err) {
      console.error('Error in getByStatus:', err);
      return response.error(res, err.message || 'Lỗi khi lấy danh sách đơn hàng');
    }
  },
  ...
  getAll: async (req, res) => {
    try {
      ...
      return response.success(res, data, 'Lấy danh sách tất cả đơn hàng thành công');
    } catch (err) {
  ...
  getById: async (req, res) => {
    try {
      ...
      return response.error(res, err.message || 'Lỗi khi lấy chi tiết đơn hàng');
    }
  },
  ...
  updateStatus: async (req, res) => {
    try {
      ...
      if (!maTTDH) {
        return response.error(res, null, 'Vui lòng cung cấp mã trạng thái đơn hàng', 400);
      }

      const data = await DonDatHangService.updateStatus(id, maTTDH, maNVDuyet, maNVGiao);
  ...
  updateBatchStatus: async (req, res) => {
    try {
      ...
      if (!orders || !Array.isArray(orders) || orders.length === 0) {
        return response.error(res, null, 'Vui lòng cung cấp danh sách đơn hàng hợp lệ', 400);
      }

      // Validate từng đơn hàng trong batch
      for (const order of orders) {
        if (!order.id || !order.maTTDH) {
          return response.error(res, null, 'Mỗi đơn hàng phải có id và maTTDH', 400);
        }
      }

      const results = await DonDatHangService.updateBatchStatus(orders);
      
      return response.success(res, results, `Cập nhật trạng thái thành công ${results.success} đơn hàng`);
    } catch (err) {
  ...
  getByCustomer: async (req, res) => {
    try {
      ...
      const data = await DonDatHangService.getByCustomer(customerId, page, limit);
      return response.success(res, data, 'Lấy danh sách đơn hàng của khách hàng thành công');
    } catch (err) {
      console.error('Error in getByCustomer:', err);
      return response.error(res, err.message || 'Lỗi khi lấy danh sách đơn hàng của khách hàng');
    }
  },
  ...
  getDetailById: async (req, res) => {
    try {
      ...
      return response.success(res, data, 'Lấy thông tin chi tiết đơn hàng thành công');
    } catch (err) {
      console.error('Error in getDetailById:', err);
      console.error('Full error:', err.stack);
      return response.error(res, err.message || 'Lỗi khi lấy thông tin chi tiết đơn hàng');
    }
  },
  ...
  getStatistics: async (req, res) => {
    try {
      ...
      return response.success(res, statistics, 'Lấy thống kê đơn hàng thành công');
    } catch (err) {
      console.error('Error in getStatistics:', err);
      return response.error(res, err.message || 'Lỗi khi lấy thống kê đơn hàng');
    }
  },
  ...
  updateDeliveryStaff: async (req, res) => {
    try {
      ...
      if (!maNVGiao || isNaN(maNVGiao)) {
        return response.error(res, null, 'Mã nhân viên giao hàng không hợp lệ', 400);
      }

      console.log(`Updating delivery staff for order ${id} with employee ${maNVGiao}`);

      const data = await DonDatHangService.updateDeliveryStaff(parseInt(id), parseInt(maNVGiao));
      
      console.log('Delivery staff updated successfully');
      return response.success(res, data, 'Cập nhật nhân viên giao hàng thành công');
    } catch (err) {
      ...
      if (err.message.includes('Chỉ có thể phân công') || 
          err.message.includes('không thuộc bộ phận giao hàng')) {
        return response.error(res, null, err.message, 400);
      }

      return response.error(res, err.message || 'Lỗi khi cập nhật nhân viên giao hàng');
    }
  },
  ...
  updateBatchStatus: async (req, res) => {
    try {
      ...
      // Validate input
      if (!orders || !Array.isArray(orders) || orders.length === 0) {
        return response.error(res, null, 'Danh sách đơn hàng không hợp lệ', 400);
      }

      // Validate each order in the array
      for (const order of orders) {
        if (!order.id || isNaN(order.id) || !order.maTTDH || isNaN(order.maTTDH)) {
          return response.error(res, null, 'Thông tin đơn hàng không hợp lệ', 400);
        }
      }
      ...
      return response.success(res, results, 'Cập nhật trạng thái đơn hàng hàng loạt thành công');
    } catch (err) {
      console.error('Error in updateBatchStatus:', err);
