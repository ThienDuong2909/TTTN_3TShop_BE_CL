...
const LoaiSPController = {
  ...
  async getAll(req, res) {
    try {
      ...
      return response.success(res, data, 'Lấy danh sách loại sản phẩm thành công');
    } catch (err) {
  ...
  async create(req, res) {
    try {
      ...
      return response.error(res, err.message, 'Thêm loại sản phẩm thất bại', 400);
    }
  },
  ...
  async delete(req, res) {
    try {
      ...
      if (!result) return response.notFound(res, 'Không tìm thấy loại sản phẩm');
